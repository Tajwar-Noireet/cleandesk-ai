  ...
  return (
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {(cards || defaultCardData).map((card, index) => {
          const baseClassName = `magic-bento-card magic-bento-card--custom ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''}`;
              ...
              ref={el => {
                ...
                const handleClick = e => {
                  ...
                  const maxDistance = Math.hypot(
                    x - (x < rect.width / 2 ? rect.width : 0),
                    y - (y < rect.height / 2 ? rect.height : 0)
                  );
